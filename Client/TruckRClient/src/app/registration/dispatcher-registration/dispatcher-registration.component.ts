...
import { RegisterDispatcherService } from 'src/infrastructure/methods/register-dispatcher/register-dispatcher.service';
import { RegisterDispatcherModel } from 'src/infrastructure/methods/register-dispatcher/register-dispatcher-model';
...
@Component({
  selector: 'app-dispatcher-registration',
  templateUrl: './dispatcher-registration.component.html',
  styleUrls: ['./dispatcher-registration.component.scss']
})
export class DispatcherRegistrationComponent implements OnInit, OnDestroy {
  ...
  confirm = ():void => {
    ...
    let model = new RegisterDispatcherModel(firstname, lastname, email, phone, login, password);

    this.registerDispatcher.register(model).subscribe((res)=>{
      if(res.successful) {
        this.notification.showSuccess("Rejestracja dyspozytora powiodła się. Link aktywacyjny został przesłany na podany email.");
      } else {
